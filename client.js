...
var targetMachineName = process.argv[2];
var localBuildFile = process.argv[3];

var targetMachine = config.targetMachines.filter(function(t) {
    return t.localName === targetMachineName;
})[0];

if (!targetMachine) {
    
    console.log('Specify machine name: ' + config.targetMachines.map(function(t) {
        return t.localName;
    }).join(', '));
    
} else {

    var targetUrl = 'http://' + targetMachine.remoteName + ':' + config.port + '/';

    var jenkinsKey = targetMachine.defaultBuild;
    ...
    (function() {
        ...
        return util.until(function() {
            return util.request({
                method: 'POST',
                uri: targetUrl + 'version',
                json: {
                    loggingDir: targetMachine.loggingDir
                }
        ...
        return util.forEach(config.databases, function(database) {
            database.server = targetMachine.localName;
